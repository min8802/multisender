...
const Multi: FC = () => {
  ...
  const switchToNetwork = async (selectedNetwork : string) => {
    try {
      ...
      } else if(selectedNetwork === "0x89") {
        console.log(selectedNetwork);
        //setMultiContract(new Contract(BSCtokenMultisenderCA, tokenMultiContractAbi, signer));
      } else if(selectedNetwork === "0xa86a") {
        console.log(selectedNetwork);
        //setMultiContract(new Contract(BSCtokenMultisenderCA, tokenMultiContractAbi, signer));
      } else if(selectedNetwork === "0xa") {
        console.log(selectedNetwork);
        //setMultiContract(new Contract(BSCtokenMultisenderCA, tokenMultiContractAbi, signer));
      } else if(selectedNetwork === "0x2105") {
        console.log(selectedNetwork);
        //setMultiContract(new Contract(BSCtokenMultisenderCA, tokenMultiContractAbi, signer));
  ...
  const networkHandler = async (e : React.ChangeEvent<HTMLSelectElement>) => {
    try {
      ...
      if(selectedNetwork === "0xa4b1") {
        ...
        const polygonChainId = "0x89"; // Arbitrum One Chain ID (42161 in decimal)
          await window.ethereum.request({
            method: "wallet_switchEthereumChain",
        ...
        const provider = new ethers.BrowserProvider(window.ethereum);
        setSigner(await provider.getSigner());
        switchToNetwork("0x89");
      
      } else if(selectedNetwork === "0xa86a") {
        const avalancheChainId = "0xa86a"; // Arbitrum One Chain ID (42161 in decimal)
          await window.ethereum.request({
            method: "wallet_switchEthereumChain",
        ...
        const provider = new ethers.BrowserProvider(window.ethereum);
        setSigner(await provider.getSigner());
        switchToNetwork("0xa86a");
     
      } else if(selectedNetwork === "0xa") {
        const optimismChainId = "0xa"; // Arbitrum One Chain ID (42161 in decimal)
          await window.ethereum.request({
            method: "wallet_switchEthereumChain",
        ...
        const baseChainId = "0x2105"; // Arbitrum One Chain ID (42161 in decimal)
          await window.ethereum.request({
            method: "wallet_switchEthereumChain",
        ...
        try {
            ...
            await window.ethereum.request({
                method: "wallet_switchEthereumChain",
                params: [{ chainId: fantomChainId }],
            });
        } catch (switchError) {
            // 네트워크가 추가되지 않은 경우, 사용자가 네트워크를 추가할 수 있도록 요청
            if ((switchError as any).code === 4902) {
                try {
                    // Fantom 네트워크 정보를 제공하여 네트워크 추가 요청
                    await window.ethereum.request({
                    ...
                    console.error("Failed to add the Fantom network:", addError);
                }
            } else {
                console.error("Failed to switch to the Fantom network:", switchError);
            }
        }
